# %%
# --- Title and Description ---

# Membrane mechanics simulation
# Based on MemberaneMechanics_Hassinger_PNAS_2017: https://www.pnas.org/doi/full/10.1073/pnas.1617705114
#The original code could be found on Rangamani Lab github: https://github.com/Rangamani-Lab/hassinger-rangamani-2017/tree/master 

# Author: Anoosh mahdavian
# July 2025
# Last Edit: [22 July 2025]

# %%
# --- Code Structure ---
# The code contains manin parts:
    #1. The physical parameter determination, 
    #2. The parametrization of arch length and area of curvature 
    #3. The set of 6 ODEs to be solved
    #4. The BCs
    #5. The initial Guess
    #6. The solver
    #7. Visualization

# %%
# --- Imports and Setup ---

import numpy as np
import matplotlib.pyplot as plt
from scipy.integrate import solve_bvp
from numpy import tanh, cos, sin

# Plot style
plt.style.use('seaborn-whitegrid')


# %%
# --- 1. Physical and Model Parameters ---

# Reference values
R0 = 20                                                 # nm, reference length
kappa0 = 320                                            # pN·nm, reference bending modulus
gamma = 50                                              # Sharpness of coat/force transition

# Coat properties
C0 = 0.02                                               # nm^-1, spontaneous curvature in coated region
c0_nd = C0 * R0                                         # dimensionless spontaneous curvature

# Coat area range
alpha_end = 7
alpha_max = 60                                         # matches disk radius R = 400 nm

# Bending modulus 
kappa_coat = 2400                                       # pN·nm
kappa_bare = 320                                        # pN·nm
kappa_coat_nd = kappa_coat / kappa0
kappa_bare_nd = kappa_bare / kappa0

# Gaussian modulus 
kappaG_coat_nd = 0
kappaG_bare_nd = 0

# Tension
lambda0=2                                              # pN.nm^-1
lambda0_nd = lambda0 * R0**2 / kappa0                    # dimensionless

# Force 
f0 = 0                                                   # pN

# Force region (like coat profile)
alpha_in = alpha_end                                     # outward ring starts
alpha_out = alpha_in + 2                                 # outward ring ends
force_area = 2 * np.pi * R0**2 * (alpha_out - alpha_in)  # in nm^2
f0_nd = (f0 / force_area) * (R0**3 / kappa0)             # dimensionless

# Pressure
p0_nd = 0                                                # dimensionless pressure/but the pressure unit in pN/nm^2


# %%
# --- 2. Coat and Force Profile Functions (One-Sided for Half-Membrane) ---

import numpy as np

# --- Smooth coat profile: active near the tip, decays toward edge (alpha_start ≡ a0) ---
def s(alpha_nd):
    return 0.5 * (1 - np.tanh(gamma * (alpha_nd - alpha_end)))

def ds(alpha_nd):
    return 0.5 * gamma * (np.tanh(gamma * (alpha_nd - alpha_end))**2 - 1)

def d2s(alpha_nd):
    tanh_term = np.tanh(gamma * (alpha_nd - alpha_end))
    return -gamma**2 * tanh_term * (1 - tanh_term**2)

# --- Spontaneous curvature profile and its derivative ---
def c_nd(alpha_nd):
    return c0_nd * s(alpha_nd)

def dc_nd(alpha_nd):
    return c0_nd * ds(alpha_nd)

# --- Bending modulus profile and its derivative ---
def kappa_nd(alpha_nd):
    return kappa_bare_nd + (kappa_coat_nd - kappa_bare_nd) * s(alpha_nd)

def dkappa_nd(alpha_nd):
    return (kappa_coat_nd - kappa_bare_nd) * ds(alpha_nd)

# --- Gaussian modulus profile and its derivatives ---
def kappaG_nd(alpha_nd):
    return kappaG_bare_nd + (kappaG_coat_nd - kappaG_bare_nd) * s(alpha_nd)

def dkappaG_nd(alpha_nd):
    return (kappaG_coat_nd - kappaG_bare_nd) * ds(alpha_nd)

def d2kappaG_nd(alpha_nd):
    return (kappaG_coat_nd - kappaG_bare_nd) * d2s(alpha_nd)

# --- Pressure profile (localized near tip) ---
def p_nd(alpha_nd):
    return p0_nd * s(alpha_nd)

# --- Actin force profile (one-sided inward pull from tip) ---
def force_profile(alpha_nd, alpha_in, alpha_out, gamma):
    """
    One-sided actin pulling force profile (decays from tip outward).
    Matches Hassinger MATLAB 'fbar' definition.
    """
    return (-f0_nd )* 0.5 * (np.tanh(gamma * (alpha_nd - alpha_in) - np.tanh(gamma * (alpha_nd - alpha_out))))

# %%
# --- 3. ODE System ---

def membrane_odes(alpha_nd, X):

    x, y, psi, h, l, lam = X


    # Property profiles
    c = c_nd(alpha_nd)
    dc = dc_nd(alpha_nd)
    k = kappa_nd(alpha_nd)
    dk = dkappa_nd(alpha_nd)
    kG = kappaG_nd(alpha_nd)
    dkG = dkappaG_nd(alpha_nd)
    d2kG = d2kappaG_nd(alpha_nd)
    

    # Optional: clip profiles to avoid instability
    x = np.where(np.abs(x) < 1e-3, 1e-3, x)
    k = np.clip(k, 1e-3, None)
    dk = np.clip(dk, -10, 10)
    


    # Gaussian curvature
    g = h**2 - (h - sin(psi)/x)**2

    # Pressure
    p = p_nd(alpha_nd)    # could be 0 or localized, defined in profile section
    
    # External force
    fbar = force_profile(alpha_nd, alpha_in, alpha_out, gamma)
    f_dot_n = fbar * np.cos(psi)
    f_dot_as = fbar * np.sin(psi)

    # ODE system
    dx = cos(psi) / x
    dy = sin(psi) / x
    dpsi = 2*h/x - sin(psi)/x**2
    dh = l/ x**2 + dc - (dk / k) * (h - c) 

    dl = (
        p / k + f_dot_n / k
        + 2*h*((h - c)**2 + lam / k)
        - 2*(h - c)*(h**2 + (h - sin(psi)/x)**2)
        - (dk / k)*l
        - (x*d2kG / k) * sin(psi)
        - (dkG / k) * cos(psi) * (2*h - sin(psi)/x)
    )

    dlam = -dk*(h - c)**2 + 2*k*(h - c)*dc - dkG*g - f_dot_as / x

    # Optional checking of solver
    if np.any(np.isnan(dx)):
        print("NaNs in dx output!")
        #print("alpha_nd:", alpha_nd)
        #print("x min:", x.min())
        #print("k min:", k.min())
    if np.any(np.isnan(dy)):
        print("NaNs in dy output!")
    if np.any(np.isnan(dpsi)):
        print("NaNs in dpsi output!")
    if np.any(np.isnan(dh)):
        print("NaNs in dh output!")
    if np.any(np.isnan(dl)):
        print("NaNs in dl output!")
    if np.any(np.isnan(dlam)):
        print("NaNs in dlam output!")


    if np.any(np.isnan(c)) or np.any(np.isnan(dc)):
        print("NaN in curvature profiles!")
    if np.any(np.isnan(h)) or np.any(np.isinf(h)):
        print("NaN or inf in h!")
    if np.max(np.abs(h)) > 100:
        print(" h is exploding!", np.max(h))
 


  
        

    return np.vstack([dx, dy, dpsi, dh, dl, dlam])
    


# %%

# --- 4. Boundary Conditions ---

def membrane_bc(X0, X1):
    x0, y0, psi0, h0, l0, lam0 = X0
    x1, y1, psi1, h1, l1, lam1 = X1

    return np.array([
        x0 - 1e-4,         # x(0) = ds, for not being zero
        psi0,              # psi(0) = 0
        l0,                # l(0) = 0
        y1,                # y(alpha_max) = 0
        psi1,              # psi(alpha_max) = 0
        lam1 - lambda0_nd  # lambda(alpha_max) = prescribed
    ])


# %%

# --- 5. Initial Guess ---

#alpha_mesh = np.linspace(1e-4, alpha_max, 10000)

# Regions: [0, alpha_end), [alpha_end, alpha_out), [alpha_out, alpha_max]
mesh1 = np.linspace(1e-4, alpha_end, 10000,endpoint=False)
mesh2 = np.linspace(alpha_end, alpha_out, 10000, endpoint=False)
mesh3 = np.linspace(alpha_out, alpha_max, 1000)
alpha_mesh = np.concatenate([mesh1, mesh2, mesh3])

# Initialize all 6 state variables: [x, y, psi, h, l, lam]
X_guess = np.zeros((6, alpha_mesh.size))

# x: radial coordinate — starts near 0, grows smoothly
X_guess[0] = alpha_mesh + 1e-4
# y: vertical position — dome-shaped bump
X_guess[1] = 0

# psi, h, l start at 0
X_guess[2] = 0
X_guess[3] = 0
X_guess[4] = 0

# set lambda ≈ lambda0_nd 
X_guess[5] = lambda0_nd 



# %%
# --- 6. Solve the BVP ---

sol = solve_bvp(
    fun=membrane_odes,
    bc=membrane_bc,
    x=alpha_mesh,
    y=X_guess,
    tol=1e-3,
    max_nodes=10000000,
    verbose=2  # Optional: detailed solver log
)

if sol.success:
    print(" BVP solver converged.")
else:
    print(" BVP solver failed.")


# %%
plt.plot(alpha_mesh, c_nd(alpha_mesh), label='Spontaneous curvature')
plt.title("Spontaneous Curvature Profile")
plt.xlabel(r'$\alpha_{nd}$')
plt.grid(True)
plt.legend()
plt.show()


# %%
# --- 7. Visualization ---

x = sol.y[0] * R0   # radial coordinate (nm)
y = sol.y[1] * R0   # vertical position (nm)

# Mirror profile for full axisymmetric shape
x_full = np.concatenate([-x[::-1], x])
y_full = np.concatenate([y[::-1], y])

# Plot membrane profile
fig, ax = plt.subplots(figsize=(6, 6))
ax.plot(x_full, y_full, lw=2)

ax.set_xlabel("r (nm)", fontsize=12)
ax.set_ylabel("z (nm)", fontsize=12)
ax.set_title(f"Membrane Profile ($\\lambda$ = {lambda0:.4f} pN/nm)", fontsize=14)
ax.set_xlim(-400, 400)
ax.set_ylim(-200, 20)
ax.set_aspect('equal')
ax.grid(True)
plt.show()

# (Optional) Plot coat/modulus profile
plt.figure()
plt.plot(alpha_mesh, c_nd(alpha_mesh), label='Spontaneous curvature')
plt.title("Coat and Modulus Profiles")
plt.xlabel(r'$\alpha_{nd}$')
plt.grid(True)
plt.legend()
plt.show()




